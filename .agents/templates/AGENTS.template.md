...
<!-- OG-AGENTS-TEMPLATE-v1
Единый служебный комментарий шаблона. Стабильный машинный маркер OG-AGENTS-TEMPLATE-v1 (первая
строка) — по нему guard в INSTALL §B.6 проверяет, что весь этот блок удалён. Удалять блок ЦЕЛИКОМ.
...
После заполнения удалить этот комментарий ЦЕЛИКОМ. Перед commit проверить, что плейсхолдеров не
...
3. Если задача затрагивает архитектуру/процесс — реестр канон-решений (`docs/architecture/ADR-INDEX.md`, если заведён в проекте; иначе `.agents/PIPELINE_ADR.md`). Противоречишь записи — стоп, нужен ADR-пересмотр, а не «улучшение по ходу».
